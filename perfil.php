...
$link = mysqli_connect("127.0.0.1","root","","yacarta");
  ...
  $infouser = mysqli_query($link,"SELECT * FROM usuarios WHERE id_use = '$id'");
  $use = mysqli_fetch_array($infouser,MYSQLI_ASSOC);

  $amigos = mysqli_query($link,"SELECT * FROM amigos WHERE de = '$id' AND para = '".$_SESSION['id']."' OR de = '".$_SESSION['id']."' AND para = '$id'");
  $ami = mysqli_fetch_array($amigos,MYSQLI_ASSOC);
  ?>
<!DOCTYPE html>
<html>
<head>
  ...
  <title><?php echo $use['nombre']; ?> | YaCarta</title>
          ...
          <div class="box box-primary">
            <div class="box-body box-profile">
              ...
              if(isset($_POST['seguir'])) {
                $add = mysqli_query($link,"INSERT INTO amigos (de,para,fecha,estado) values ('".$_SESSION['id']."','$id',now(),'0')");
              ...
              if(isset($_POST['seguirdirecto'])) {
                $add = mysqli_query($link,"INSERT INTO amigos (de,para,fecha,estado) values ('".$_SESSION['id']."','$id',now(),'1')");
                if($add) {echo '<script>window.location="perfil.php?id='.$id.'"</script>';}
              }
              ?>
              ...
              if(isset($_POST['dejarseguir'])) {
                $add = mysqli_query($link,"DELETE FROM amigos WHERE de = '$id' AND para = '".$_SESSION['id']."' OR de = '".$_SESSION['id']."' AND para = '$id'");
                if($add) {echo '<script>window.location="perfil.php?id='.$id.'"</script>';}
              }
              ?>
