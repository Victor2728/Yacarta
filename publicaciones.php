...
$CantidadMostrar=5;
	...
	$TotalReg       =mysqli_query($link,"SELECT * FROM publicaciones");
  ...
  //Consulta SQL
	...
	$consulta=mysqli_query($link,$consultavistas);
	while ($lista=mysqli_fetch_array($consulta)) {

		$userid = mysqli_real_escape_string($link,$lista['usuario']);

		$usuariob = mysqli_query($link,"SELECT * FROM usuarios WHERE id_use = '$userid'");
    $use = mysqli_fetch_array($usuariob,MYSQLI_ASSOC);

    $fotos = mysqli_query($link,"SELECT * FROM fotos WHERE publicacion = '$lista[id_pub]'");
    $fot = mysqli_fetch_array($fotos,MYSQLI_ASSOC);
	?>
	<!-- START PUBLICACIONES -->
          <!-- Box Comment -->
          <div class="box box-widget">
            ...
            <div class="box-body">
              ...
              <br><br>
              <?php 
              $numcomen = mysqli_num_rows(mysqli_query($link,"SELECT * FROM comentarios WHERE publicacion = '".$lista['id_pub']."'"));
              ?>
              <!-- Social sharing buttons -->
            <ul class="list-inline">

              <?php
              $query = mysqli_query($link,"SELECT * FROM likes WHERE post = '".$lista['id_pub']."' AND usuario = ".$_SESSION['id']."");
            ...
            <?php 
            $comentarios = mysqli_query($link,"SELECT * FROM comentarios WHERE publicacion = '".$lista['id_pub']."' ORDER BY id_com desc LIMIT 2");
            while($com=mysqli_fetch_array($comentarios)){
              $usuarioc = mysqli_query($link,"SELECT * FROM usuarios WHERE id_use = '".$com['usuario']."'");
              ...
              <br>
                ...
                <input type="text" class="enviar-btn form-control input-sm" style="width: 700px;" placeholder="Escribe un comentario" name="comentario" id="comentario-<?php  echo $lista['id_pub'];?>">
